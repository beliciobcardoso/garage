...
if ! docker compose ps --format json | grep -q '"State":"running"'; then
    ...
    echo -e "Por favor, inicie o container primeiro rodando: ${YELLOW}docker compose up -d${NC}"
    exit 1
fi
...
if docker compose exec -T garage /garage bucket info "$BUCKET_NAME" >/dev/null 2>&1; then
    echo -e "${YELLOW}Aviso: O bucket '${BUCKET_NAME}' já existe. Ignorando criação.${NC}"
else
    echo -e "${BLUE}[2/3] Criando o bucket S3...${NC}"
    docker compose exec -T garage /garage bucket create "$BUCKET_NAME" > /dev/null
fi

# 5. Associar a chave ao bucket
echo -e "${BLUE}[3/3] Vinculando permissões de leitura/escrita...${NC}"
docker compose exec -T garage /garage bucket allow "$BUCKET_NAME" --key "$ACCESS_KEY" --read --write > /dev/null
